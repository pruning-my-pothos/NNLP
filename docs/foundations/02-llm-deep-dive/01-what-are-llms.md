...
flowchart LR
    ...
    classDef output fill:#DCFCE7,stroke:#15803D,stroke-width:2px,color:#0F2A1A;
...
-   **Control Stochasticity**: Carefully choose parameters like `temperature` and `top-p` sampling. See [Temperature](./temperature.md) and [Parameter: Top-k vs. Top-p Sampling](./parameter-top-k-vs-top-p-sampling.md) for details.
...
Explore the foundational architecture of LLMs in **[Transformer Architecture (for LLMs)](./transformer-architecture.md)**.
