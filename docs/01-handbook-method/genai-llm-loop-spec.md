---
title: "GenAI & LLM Handbook Loop: Normative Process Model"
archetype: "method"
status: "active"
...
Define the required GenAI & LLM Handbook workflow so teams can adopt it consistently. This ensures repeatable, auditable execution from problem definition to acceptance.
:::

## Overview

The GenAI & LLM Handbook Loop is an 8-step iterative process designed to make AI-assisted work predictable and accountable. Each step focuses on producing a specific, reviewable artifact. This document outlines the normative requirements for each stage of the loop.
...
A basic understanding of the GenAI & LLM Handbook core skills (Logic, Language, Systems, Sentences) is highly recommended.
:::

- **Artifacts**: Understanding of the core GenAI & LLM Handbook artifacts (Intent Spec, Constraint Spec, Delegation Contract, etc.).
...
## The GenAI & LLM Handbook Loop (8 Steps)
...
-   **Output**: Discovery Brief artifact ([template](/docs/06-templates/discovery-brief-template)).

### 2. Intent Spec

-   **Purpose**: Define the desired outcome and success criteria.
-   **Input**: Discovery Brief.
-   **Output**: Intent Spec artifact ([template](/docs/06-templates/intent-spec-template)).

### 3. Constraint Spec

-   **Purpose**: Encode non-negotiable boundaries, technical requirements, and guardrails.
-   **Input**: Intent Spec.
-   **Output**: Constraint Spec artifact ([template](/docs/06-templates/constraint-spec-template)).

### 4. Delegation Contract

-   **Purpose**: Define what the AI is allowed (and not allowed) to do.
-   **Input**: Intent Spec, Constraint Spec.
-   **Output**: Delegation Contract artifact ([template](/docs/06-templates/delegation-contract-template)).
...
-   **Output**: Generation Request artifact ([template](/docs/06-templates/generation-request-template)), AI draft output.
...
-   **Output**: Acceptance Criteria artifact ([template](/docs/06-templates/acceptance-criteria-template)).
...
To claim adherence to the GenAI & LLM Handbook Standard, a workflow **MUST** demonstrate:
...
Explore [Generation Requests](/docs/01-handbook-method/generation-requests) for structuring prompts.
