...
-   **Artifacts**: Basic understanding of the GenAI Project Lifecycle.
...
## Specify with the GenAI Project Lifecycle

The GenAI Project Lifecycle is fundamentally a specifying process. Each stage builds a richer, more precise specification for the AI.
...
flowchart LR
    ...
    B --> C{Intent Spec}
    C --> D{Constraint Spec}
    D --> E[Generation Request (Precise Prompt)]
    E --> F[AI Output]
    F --> G{Review & Interrogation}
    G --> H{Acceptance Criteria}
    H --> I[Accepted Artifact]
...
3.  **Generation Request (Precise Prompt):**

    ```markdown
    "Generate a TypeScript function `sanitizeHtmlInput(input: string): string`.
    Adhere to the attached Intent Spec and Constraint Spec.
